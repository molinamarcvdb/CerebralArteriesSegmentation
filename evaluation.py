...
def eval_metrics(args):
    ...
    TESTING. It assumes that inference has been run over all images 
    belonging to the testing set of a model, and stored in args.logdir/test/data.
    ...
    progress_dir = os.path.join(args.logdir, 'test','progress')
    maybe_mkdir_p(progress_dir)

    # Set directories tor ead preds//labels 
    test_dir_preds = os.path.join(args.logdir, 'test','preds')
    test_dir_labels = os.path.join(args.logdir, 'test','labels')      
