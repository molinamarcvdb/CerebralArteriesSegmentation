...
default_3D_augmentation_params = {
    ...
    "do_elastic": False,
    "elastic_deform_alpha": (0., 500.),
    "elastic_deform_sigma": (5., 12.),
    "p_eldef": 0.2,
    ...
    "do_rotation": True,
    "rotation_x": (-7.5 / 360 * 2. * np.pi, 7.5 / 360 * 2. * np.pi),
    "rotation_y": (-7.5 / 360 * 2. * np.pi, 7.5 / 360 * 2. * np.pi),
    "rotation_z": (-7.5 / 360 * 2. * np.pi, 7.5 / 360 * 2. * np.pi),
    "rotation_p_per_axis": 1,
    "p_rot": 0.2,

    "random_crop": False,
    "random_crop_dist_to_border": 65,
...
def get_default_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                             border_val_seg=-1, pin_memory=True, seeds_train=None, seeds_val=None, regions=None):
    ...
    tr_transforms.append(SpatialTransform(
                                patch_size_spatial, 
                                patch_center_dist_from_border = params.get("random_crop_dist_to_border"), 
...
def get_default_test_augmentation(dataloader_test, patch_size, params=default_3D_augmentation_params,
                                    border_val_seg=-1, pin_memory=True, seeds_train=None, seeds_val=None, regions=None):
    # don't do color augmentations while in 2d mode with 3d data because the color channel is overloaded!!
    if params.get("dummy_2D") is not None and params.get("dummy_2D"):
        # tr_transforms.append(Convert3DTo2DTransform())
    ...
    # test_transforms.append(SpatialTransform(
    #                             patch_size_spatial, 
    #                             patch_center_dist_from_border=patch_size[0]//2, 
    #                             do_elastic_deform= False,
    #                             do_rotation=False,  
    ...
    test_transforms.append(RemoveLabelTransform(-1, 0))
    if params.get("selected_data_channels") is not None:
        test_transforms.append(DataChannelSelectionTransform(params.get("selected_data_channels")))
    if params.get("selected_seg_channels") is not None:
        test_transforms.append(SegChannelSelectionTransform(params.get("selected_seg_channels")))

    # if params.get("move_last_seg_chanel_to_data") is not None and params.get("move_last_seg_chanel_to_data"):
    #     val_transforms.append(MoveSegAsOneHotToData(1, params.get("all_segmentation_labels"), 'seg', 'data'))

    test_transforms.append(RenameTransform('seg', 'target', True))

    # if regions is not None:
    #     val_transforms.append(ConvertSegmentationToRegionsTransform(regions, 'target', 'target'))

    test_transforms.append(NumpyToTensor(['data', 'target'], 'float'))
    test_transforms = Compose(test_transforms)

    # batchgenerator_val = SingleThreadedAugmenter(dataloader_val, val_transforms)
    batchgenerator_test = MultiThreadedAugmenter(dataloader_test, test_transforms, max(params.get('num_threads') // 2, 1),
                                                params.get("num_cached_per_thread"), seeds=seeds_val,
                                                pin_memory=pin_memory)
    
    return batchgenerator_test